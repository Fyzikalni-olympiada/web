	...
	if (isset($_POST['participant_solution_count']) && is_array($_POST['participant_solution_count'])) {
		...
		for ($i=1; $i<=$participant_count; $i++) {
			if (!isset($participant_solution_count[$i]) || !is_numeric($participant_solution_count[$i])) {
				...
				$chyba .= 'Vyplòte správný poèet odevzdaných úloh soutìžícího è.'.$i.'.<br />';
			}
		}
	} else {
		$chyba .= 'Neoèekávaný vstup.<br />';
...
echo '
<p>
Vyplòte prosím následující formuláø. Žádáme Vás pøedevším o&nbsp;kontakt na Vás a&nbsp;na soutìžící
...
<div>
<label for="participant_count">Poèet soutìžících:</label> <input type="text" id="participant_count" name="participant_count" size="2" value="' . $participant_count . '">
</div>
...
<input type="submit" value="Snížit poèet soutìžících" name="step2-decrease">
<input type="submit" value="Zvýšit poèet soutìžících" name="step2-increase">
...
if (isset($_POST['step2']) && empty($chyba)) {
	...
	echo '
<p>
Následující seznam mùžete vytisknout a pøiložit k&nbsp;odevzdávaným úlohám.
</p>
