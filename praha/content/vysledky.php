...
echo '
<p>
Zde uveřejňujeme výsledkové listiny domácího kola a&nbsp;pražského krajského kola.
</p>
<div id="vysledky">
';
$soubory = array();
...
<h3>' . $rocnik . '. ročník &ndash; '.(1958+$rocnik).'/'.(1959+$rocnik).'</h3>
	...
	foreach ($odkaz_text_sub_array as $kolo => $odkaz_text_subsub_array) {
		...
		foreach ($odkaz_text_subsub_array as $kategorie => $odkaz_text) {
			...
			echo $oddelovac.'<a href="' . odkaz('praha/content/listina.php') . '&rocnik='.$rocnik.'&kolo='.$kolo.'&kategorie='.$kategorie . '" title="' .$kolo_text .' '. $odkaz_text . '">'.$odkaz_text.'</a>';
		}
		echo '
		</dd>';
	}
