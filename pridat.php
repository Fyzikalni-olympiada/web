...
if (isset($_POST["ok"])) { 
	...
	$_POST["file_id"] = $GLOBALS['mysql']->odstran_problemy($_POST["file_id"]);
	...
	if (empty($prokohoArray)) {
		$chyba .= "Není určeno pro koho novinka je.<BR>";
	} else {
	...
	if ( $_FILES['foto']['size'] !== 0 ) {       //odeslán i obrázek
		if ( $_FILES['foto']['type'] == "image/jpeg"
			...
			if (!file_exists("pic/$name") and ($_FILES['foto']['size'] <= 1050000)){
				...
				$pic_id = '';
				// zjistíme pic_id (pokud obrázek existuje, změníme ho)
				if (isset($_GET['id'])) {
					$GLOBALS['mysql']->query('
				...
				$values = "'" . $pic_id . "', '" . $name . "'";
				$_POST["align"]  != "" ? $values .= ", " . db::escape_string($_POST["align"]) : $values .= ", 'left'";
				$_POST["hspace"] != "" ? $values .= ", " . db::escape_string($_POST["hspace"]) : $values .= ", NULL";
				$_POST["vspace"] != "" ? $values .= ", " . db::escape_string($_POST["vspace"]) : $values .= ", NULL";
				$_POST["alt"]    != "" ? $values .= ", "  . db::escape_string($_POST["alt"]) : $values .= ", ''";
				
				$query_img = "REPLACE INTO `" . TABLE_IMAGES . "` ( `id` , `filename` , `align` , `hspace` , `vspace` , `alt` ) VALUES ( " . $values . " ) ";
			} else {
				$chyba .= "Obrázek se nepodařilo uploadovat, je příliš veliký.";
			}
		} else {
	...
	if ($chyba == "") {
		...
		$_POST["titleline"] = db::escape_string($_POST["titleline"]);
		$_POST["name"] = db::escape_string($_POST["name"]);
		$_POST["email"] = db::escape_string($_POST["email"]);
		$_POST["file_id"] = db::escape_string($_POST["file_id"]);
...
<?php
$query = "
	SELECT f.id AS file_id, ms.id AS id, ms.type AS type, ms.href AS href, ms.name AS name, ms.title AS title, f.filename AS filename
...
while ( $row = mysql_fetch_assoc($result) ) { //hlavní menu
	$options .= "<option value=\"$row[file_id]\"> $row[name] </option>\r\n";
	echo "
if (document.sign.linkpredef.value=='$row[file_id]'){";
	if ($row['type'] == 'file') {
		echo "
	...
	while ( $row2 = mysql_fetch_array($result2) ) { //submenu
		$options .= "<option value=\"$row2[file_id]\">|-- $row2[name] </option>\r\n";
		echo "
if (document.sign.linkpredef.value=='$row2[file_id]'){";
		if ($row2['type'] == 'file') {
			echo "
    ...
    document.sign.linktitle.value='$row2[title]';
		...
		while ( $row3 = mysql_fetch_array($result3) ) { //subsubmenu
			$options .= "<option value=\"$row3[file_id]\">&nbsp;&nbsp;&nbsp;|-- $row3[name] </option>\r\n";
			echo "
if (document.sign.linkpredef.value=='$row3[file_id]'){";
			if ($row3['type'] == 'file') {
				echo "
...
<div id="form_pridat">
<form name="sign" id="sign" action="<? echo $_SERVER['PHP_SELF'] . $get_url ?>" method="post" ENCTYPE="multipart/form-data" onsubmit="checkit()">
...
<tr valign="top">
  ...
  <tr>
    <td align="right">Nová HP:</td>
	<td><select name="file_id" tabindex="3"><?php
	echo $options;
      ?>
      </select></td>
  </tr>
...
<tr valign="top">
  ...
  <tr>
	...
	<td>
		...
		<input type="checkbox" name="ucit" tabindex="10" <?php echo isset($_POST["ucit"]) && $_POST["ucit"] ? "checked" : ""; ?>>učitel
